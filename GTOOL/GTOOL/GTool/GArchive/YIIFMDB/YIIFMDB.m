...
//     表名，不可以为nil   ,AAA_111    类型     数字不能在前
    ...
    if (!isSuccess) {
        [self log:[NSString stringWithFormat:@"create Table失败，错误的sqliteString = %@", sqliteString]];
    }
    return isSuccess;
}
...
- (BOOL)deleteFromTable:(NSString *)tableName whereParameters:(YIIParameters *)parameters {
    ...
    BOOL isSuccess = [self.database executeUpdate:sqliteString];
    if (!isSuccess) {
        [self log:[NSString stringWithFormat:@"建表失败，错误的sqliteString = %@", sqliteString]];
    }
    return isSuccess;
}
...
#pragma mark - 更改数据
//@"update TABLE_NAME set KEY = VALUE ,KEY = VALUE where primaryKey = 111"
- (BOOL)updateTable:(NSString *)tableName dictionary:(NSDictionary *)dictionary whereParameters:(YIIParameters *)parameters {
    ...
    if (values.count > 0) {
        ...
        BOOL isSuccess =  [self.database executeUpdate:sqliteString withArgumentsInArray:values];
        if (!isSuccess) {
            [self log:[NSString stringWithFormat:@"update失败，错误的sqliteString = %@", sqliteString]];
        }
        return   isSuccess;
...
- (BOOL)alterTable:(NSString *)tableName column:(NSString *)column type:(YIIFMDBValueType)type {
    ...
    BOOL isSuccess =  [self.database executeUpdate:sqliteString];
    if (!isSuccess) {
        [self log:[NSString stringWithFormat:@"update Table失败，错误的sqliteString = %@", sqliteString]];
    }
    return isSuccess;
}
